...
sub-layer collapses with the identity mix (``_make_identity_pre_mix``) and
every following sub-layer collapses with the previous one's freshly
computed ``pre`` (the ``forward_with_pre_mix`` seam); the final hidden
state is collapsed once by the last block (``collapse``).  ``HcPre`` and
``HcPost`` are otherwise the DSV4 modules: one shared FP32 cast for mix
generation and stream collapse in ``forward`` (splitting the casts is
forward-equivalent but changes BF16 backward rounding, visible in the
frozen trajectory).
"""
...
class HcPre(Module):
    ...
    def forward_with_pre_mix(self, x, pre_mix=None):
        """Collapse with the caller-supplied stream mix of the single-pass stack.

        ``pre_mix=None`` falls back to the local mix generated from ``x``
        itself (used by tests and the first sub-layer equivalence check).
        Returns the collapsed stream plus the post/comb factors and the
        freshly computed ``pre`` that the next sub-layer collapses with.
        """
    ...
    def forward(self, x):
        """One-shot mix-and-collapse (the standalone mHC step)."""
        shape, dtype = x.size(), x.dtype
        x = x.flatten(2).float()
        rsqrt = torch.rsqrt(x.square().mean(-1, keepdim=True) + self.norm_eps)
        mixes = F.linear(x, self.hc_fn.float()) * rsqrt
