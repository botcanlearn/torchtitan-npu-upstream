    ...
    [
        pytest.param(
            ...
            marks=pytest.mark.xfail(
                strict=True,
                reason=(
                    "Triton-Ascend compiles its launcher with -std=c++17 but this torch "
                    "(2.14) requires C++20, so the test-only Triton kernel "
                    "`_fill_indices_kernel` (tests/reference_moe.py, the MoE routing of "
                    "the grouped-mm path) cannot compile on this toolchain. Re-enable "
                    "once Triton-Ascend is updated."
                ),
            ),
        ),
        False,
...
def test_param_swap(device, weight_config, act_config, sqnr_threshold, use_grouped_mm):
    ...
    if isinstance(weight_config, BlockMXQuantizeConfig) and not use_grouped_mm:
        pytest.skip("BlockMX requires bfloat16 (use_grouped_mm=True)")
    ...
    for _ in range(1):
        ...
        for (cur_name, cur_param), (prev_name, prev_param) in zip(
            ...
            assert cur_name == prev_name, f"Parameter order changed: {cur_name} vs {prev_name}"
            assert type(cur_param) is type(prev_param), (
