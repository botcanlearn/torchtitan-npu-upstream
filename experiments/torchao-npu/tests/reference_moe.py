...
#
# This file is copied from torchao,
# https://github.com/pytorch/ao/blob/v0.17.0/test/prototype/moe_training/reference_moe.py
# The code is modified to run on NPU.
#
...
def _permute(x, num_tokens_per_expert, ep_degree, num_local_experts):
    ...
    with torch.no_grad():
        (
            ...
            use_cpu=not torch.npu.is_available(),
        )
...
def _run_experts_grouped_mm(
    ...
    h = F.silu(torch._grouped_mm(x.bfloat16(), gate_proj.bfloat16().transpose(-2, -1), offs=offsets))
    h = h * torch._grouped_mm(x.bfloat16(), up_proj.bfloat16().transpose(-2, -1), offs=offsets)
...
class GroupedExperts(nn.Module):
    ...
    def forward(
        ...
        if self.use_grouped_mm:
            if not isinstance(self.gate_proj, DTensor) or "ep" not in self.gate_proj.device_mesh.mesh_dim_names:
...
class TokenChoiceTopKRouter(nn.Module):
    ...
    def _debug_force_load_balance_routing(self, scores: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        selected_experts_indices = (
            torch.arange(n_tokens * self.top_k, device=scores.device, dtype=torch.int64).reshape(n_tokens, self.top_k)
            % self.num_experts
        )
    ...
    def _get_node_limited_routing_scores(
        ...
        scores_for_choice = scores_grouped.masked_fill(group_mask.unsqueeze(-1), float("-inf")).view(
            -1, self.num_experts
        )
