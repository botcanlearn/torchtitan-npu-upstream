...
description: "当需要将 torchtitan-npu 与上游 torchtitan 指定分支进行同步（sync/adapt/rebase）时使用：读取 docs/community/versioning_policy.md 的分支同步表，对比旧基线与目标提交，生成 torchtitan_changes.md，分级展示 torchtitan 主要改动点，完成 torchtitan_npu 适配，并在结束后提醒更新 versioning_policy.md。"
---
...
首先读取 [docs/community/versioning_policy.md](../../../docs/community/versioning_policy.md)。
...
- 在 `docs/community/versioning_policy.md` 的 `分支同步表` 中匹配 `current_branch`。
...
同步任务结束时，必须提醒用户更新
`docs/community/versioning_policy.md`（`分支同步表`）：
...
| 项目 | 路径 |
|---------|------|
| 分支映射事实源 | `docs/community/versioning_policy.md` |
