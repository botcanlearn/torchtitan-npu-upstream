...
**SwapOptimizer 下中间 DCP 保存和续训是否支持完整优化器状态？**

已支持。启用 SwapOptimizer 时，中间 DCP checkpoint 会保存 CPU 侧真实
optimizer state；resume 时会恢复 CPU cache 并重建 device 侧 zero-storage
placeholder，可用于完整训练状态断点续训。
