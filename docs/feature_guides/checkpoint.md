...
提供，在模型的 `config_registry.py` 中设置，也可以通过 CLI 覆盖。
...
将 DCP checkpoint 合并为单个 PyTorch `.pt` 文件：
...
HF safetensors 到 DCP 的离线转换依赖模型的 state dict adapter 和对应模型入口。
当前仓库中的 `scripts/checkpoint_conversion/convert_from_hf.py` 仍保留旧版
`TrainSpec` 调用方式，对已经迁移到 `config_registry.py` 的模型不一定可直接使用。
对于 DeepSeek-V3.2、DeepSeek-V4、Qwen3 等当前模型，优先使用
`checkpoint.initial_load_in_hf=True` 在训练启动时直接读取 HF 权重。

## 常见问题

**为什么配置了 `initial_load_path` 但没有加载？**

通常有两个原因：一是没有设置 `checkpoint.enable=True`；二是当前
`{dump_folder}/{checkpoint.folder}` 已经存在可用 checkpoint，框架优先从当前
输出目录续训。

**为什么最后一步 checkpoint 不能完整续训？**

`last_save_model_only=True` 时，最后一步只保存模型权重，不包含优化器、
学习率调度器和训练状态。需要完整续训时，请使用中间 DCP checkpoint，或在配置中将
`last_save_model_only=False`。

**DCP 和 HF 权重应该怎么选？**

断点续训、故障恢复和分布式重分片优先用 DCP；从开源模型初始化或导出给推理生态时用
HF safetensors。

**SwapOptimizer 下中间 DCP 保存或续训会报 `NotImplementedError`？**

`deepseek_v32`、`deepseek_v4` 等模型默认启用 `optimizer.swap_optimizer=True`，其 state 中含
`None` 项，目前不能被 `torch.distributed.checkpoint` 的优化器扁平化逻辑序列化，典型报错：

```
NotImplementedError: Flattening optimizer state_dict only supports tensor, int, float, dict states now. Type is <class 'NoneType'>.
```

触发位置有两处：

- `interval` 触发的中间 step 保存（含完整优化器状态）。
- 任何 `dcp.load`：因为 plan 阶段也会调用优化器的 `state_dict()`。

短期 workaround：

- 只用 `last_save_model_only=True`（默认就是 True）做模型权重落盘，保存时不要触发中间
  step（拉大 `interval` 或减少 `training.steps`）。
- Resume / `initial_load_path` 加载时显式排除优化器和训练状态：
  `--checkpoint.exclude_from_loading optimizer,lr_scheduler,dataloader,train_state`。

这是当前框架与 SwapOptimizer 的兼容性限制，待 SwapOptimizer 把 None state 改成空张量
或上游 DCP 支持 NoneType 后可恢复完整状态保存。
